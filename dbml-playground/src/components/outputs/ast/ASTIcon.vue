<template>
  <span class="inline-flex items-center justify-center w-4 h-4 flex-shrink-0" :class="iconClass">
    <!-- Database Icon - Professional multi-layer database cylinder -->
    <svg v-if="type === 'database'" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
    ...
    <!-- Table Icon - dbdocs style -->
    <svg v-else-if="type === 'table'" viewBox="0 0 11 9" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
      <path fill-rule="evenodd" clip-rule="evenodd" d="M8.48864 0.818182H1.63636C1.18449 0.818182 0.818182 1.18449 0.818182 1.63636V7.36364C0.818182 7.81551 1.18449 8.18182 1.63636 8.18182H8.48864C8.94051 8.18182 9.30682 7.81551 9.30682 7.36364V1.63636C9.30682 1.18449 8.94051 0.818182 8.48864 0.818182ZM1.63636 0C0.732625 0 0 0.732625 0 1.63636V7.36364C0 8.26737 0.732625 9 1.63636 9H8.48864C9.39237 9 10.125 8.26737 10.125 7.36364V1.63636C10.125 0.732625 9.39237 0 8.48864 0H1.63636Z" fill="currentColor"/>
      <path d="M0 1.63636C0 0.732625 0.732625 0 1.63636 0H8.48864C9.39237 0 10.125 0.732625 10.125 1.63636V2.25H0V1.63636Z" fill="currentColor"/>
    </svg>

    <!-- Column/Field Icon - dbdocs style -->
    <svg v-else-if="type === 'column' || type === 'column-pk' || type === 'column-unique' || type === 'column-required'" viewBox="0 0 12 10" height="10" width="12" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
      <path fill-rule="evenodd" clip-rule="evenodd" d="M9.43182 0.909091H1.81818C1.3161 0.909091 0.909091 1.3161 0.909091 1.81818V8.18182C0.909091 8.6839 1.3161 9.09091 1.81818 9.09091H9.43182C9.9339 9.09091 10.3409 8.6839 10.3409 8.18182V1.81818C10.3409 1.3161 9.9339 0.909091 9.43182 0.909091ZM1.81818 0C0.814028 0 0 0.814028 0 1.81818V8.18182C0 9.18597 0.814028 10 1.81818 10H9.43182C10.436 10 11.25 9.18597 11.25 8.18182V1.81818C11.25 0.814028 10.436 0 9.43182 0H1.81818Z" fill="currentColor"/>
      <path d="M0 1.81818C0 0.814028 0.814028 0 1.81818 0H9.43182C10.436 0 11.25 0.814028 11.25 1.81818V2.5H0V1.81818Z" fill="currentColor"/>
      <rect x="2.99994" y="2.5" width="2.22222" height="6.58889" fill="currentColor"/>
      <!-- Primary key indicator -->
      <circle v-if="type === 'column-pk'" cx="8.5" cy="5.5" r="1.2" fill="currentColor" opacity="0.8"/>
      <!-- Unique indicator -->
      <circle v-if="type === 'column-unique'" cx="8.5" cy="5.5" r="1" stroke="currentColor" stroke-width="0.8" fill="none"/>
      <!-- Required indicator -->
      <text v-if="type === 'column-required'" x="8.5" y="6.5" text-anchor="middle" font-size="3" fill="currentColor" font-weight="bold">!</text>
    </svg>

    <!-- Index Icon -->
    <svg v-else-if="type === 'index'" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" stroke="currentColor"/>
    </svg>

    <!-- Enum Icon - "E" in a gray box -->
    <svg v-else-if="type === 'enum'" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
      <rect x="1" y="1" width="14" height="14" rx="2" fill="currentColor" opacity="0.2" stroke="currentColor" stroke-width="1"/>
      <text x="8" y="11" text-anchor="middle" font-size="10" font-weight="bold" fill="currentColor">E</text>
    </svg>

    <!-- Ref Icon - dbdocs style -->
    <svg v-else-if="type === 'ref'" fill="none" viewBox="0 0 64 64" height="16" width="16" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
      <ellipse cx="12.8" cy="30.8" rx="4.8" ry="4.8" fill="currentColor"/>
      <path fill-rule="evenodd" clip-rule="evenodd" d="M12.8008 30.7999C12.8008 29.4744 13.8753 28.3999 15.2008 28.3999H53.6008C54.9263 28.3999 56.0008 29.4744 56.0008 30.7999C56.0008 32.1254 54.9263 33.1999 53.6008 33.1999H15.2008C13.8753 33.1999 12.8008 32.1254 12.8008 30.7999Z" fill="currentColor"/>
      <path fill-rule="evenodd" clip-rule="evenodd" d="M52.9146 14.7203C53.8423 15.667 53.8268 17.1865 52.8801 18.1142L40.2295 30.5103L52.8801 42.9065C53.8268 43.8342 53.8423 45.3537 52.9146 46.3004C51.9869 47.2472 50.4674 47.2626 49.5207 46.3349L35.1207 32.2246C34.66 31.7731 34.4004 31.1553 34.4004 30.5103C34.4004 29.8654 34.66 29.2475 35.1207 28.7961L49.5207 14.6858C50.4674 13.7581 51.9869 13.7735 52.9146 14.7203Z" fill="currentColor"/>
    </svg>

    <!-- Project Icon - Clean folder -->
    <svg v-else-if="type === 'project'" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
    ...
    <!-- TableGroup Icon - dbdocs style -->
    <svg v-else-if="type === 'tableGroup'" fill="none" viewBox="0 0 13 12" height="12" width="13" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
      <path fill-rule="evenodd" clip-rule="evenodd" d="M0 1.70455C0 0.901222 0.651222 0.25 1.45455 0.25H7.54545C8.34878 0.25 9 0.901222 9 1.70455V2.25V3.75H11.5455C12.3488 3.75 13 4.40122 13 5.20455V5.75V10.2955C13 11.0988 12.3488 11.75 11.5455 11.75H5.45455C4.65122 11.75 4 11.0988 4 10.2955V8.25H1.45455C0.651222 8.25 0 7.59878 0 6.79545V2.25V1.70455ZM4.72727 8.25H4.75V7.52273H4.72727V5.75H8.27273H9H12.2727V10.2955C12.2727 10.6971 11.9471 11.0227 11.5455 11.0227H5.45455C5.05288 11.0227 4.72727 10.6971 4.72727 10.2955V8.25ZM8.27273 3.75H5.45455C4.65122 3.75 4 4.40122 4 5.20455V5.75V7.52273H1.45455C1.05288 7.52273 0.727273 7.19712 0.727273 6.79545V2.25H8.27273V3.75Z" fill="currentColor"/>
    </svg>

    <!-- TablePartial Icon -->
    <svg v-else-if="type === 'tablePartial'" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4">
