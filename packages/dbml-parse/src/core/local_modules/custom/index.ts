...
export const customModule: LocalModule = {
  ...
  nodeFullname (compiler: Compiler, node: SyntaxNode): Report<string[] | undefined> | Report<PassThrough> {
    ...
    if (node.name) {
      return new Report(undefined, [
        new CompileError(CompileErrorCode.UNEXPECTED_NAME, 'A custom field shouldn\'t have a name', node.name),
      ]);
    }
    return new Report(undefined);
  },
  ...
  nodeAlias (compiler: Compiler, node: SyntaxNode): Report<string | undefined> | Report<PassThrough> {
    ...
    if (node.alias) {
      return new Report(undefined, [
        new CompileError(CompileErrorCode.UNEXPECTED_NAME, 'A custom field shouldn\'t have an alias', node.alias),
      ]);
    }
    return new Report(undefined);
  },
  ...
  nodeSettings (compiler: Compiler, node: SyntaxNode): Report<Settings> | Report<PassThrough> {
    ...
    if (node.attributeList) {
      return new Report({}, [
        new CompileError(CompileErrorCode.UNEXPECTED_SETTINGS, 'A custom field shouldn\'t have a setting list', node.attributeList),
      ]);
    }
    return new Report({});
  },
