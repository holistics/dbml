...
import { SymbolKind, createNodeSymbolIndex } from '@/core/validator/symbol/symbolIndex';
import { getSymbolKind } from '@/core/validator/symbol/utils';
...
export function lookupAndBindInScope (
  ...
  for (const curSymbolInfo of symbolInfos) {
    ...
    if (!symbol) {
      return [new CompileError(CompileErrorCode.BINDING_ERROR, `${kind} '${name}' does not exist in ${curName === undefined ? 'global scope' : `${curKind} '${curName}'`}`, node, context.filepath)];
    }
