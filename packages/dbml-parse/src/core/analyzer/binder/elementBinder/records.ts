...
export default class RecordsBinder implements ElementBinder {
  ...
  // FIXME: bind the records' name: `<schema>.<table>(<column>, <column>)` or `(<column>, <column>)`
  bind (): CompileError[] {
  ...
  // FIXME: scan for member access like `<schema>.<enum>.<enumField>` in function applications
