...
describe('[example - record] multiple records blocks', () => {
  test('should handle multiple records blocks for the same table with different columns', () => {
    ...
    if ('age' in db.records[0].values[0]) {
      expect(db.records[0].values[0].age).toEqual({ type: 'integer', value: null });
    }
    ...
    if ('age' in db.records[0].values[1]) {
      expect(db.records[0].values[1].age).toEqual({ type: 'integer', value: null });
    }
    ...
    if ('name' in db.records[0].values[2]) {
      expect(db.records[0].values[2].name).toEqual({ type: 'string', value: null });
    }
    expect(db.records[0].values[2].age).toEqual({ type: 'integer', value: 25 });

    expect(db.records[0].values[3].id).toEqual({ type: 'integer', value: 4 });
    if ('name' in db.records[0].values[3]) {
      expect(db.records[0].values[3].name).toEqual({ type: 'string', value: null });
    }
    expect(db.records[0].values[3].age).toEqual({ type: 'integer', value: 30 });
  });
  ...
  test('should handle multiple records blocks, one with explicit columns and one without', () => {
    ...
    expect(db.records[0].values[0].id).toEqual({ type: 'integer', value: 1 });
    expect(db.records[0].values[0].title).toEqual({ type: 'string', value: 'First post' });
    // content column may not exist on this row, or may be null
    if ('content' in db.records[0].values[0]) {
      expect(db.records[0].values[0].content).toEqual({ type: 'string', value: null });
    }

    // Second row from records posts(id, title, content)
    expect(db.records[0].values[1].id).toEqual({ type: 'integer', value: 2 });
    expect(db.records[0].values[1].title).toEqual({ type: 'string', value: 'Second post' });
    expect(db.records[0].values[1].content).toEqual({ type: 'string', value: 'Content of second post' });
  });
