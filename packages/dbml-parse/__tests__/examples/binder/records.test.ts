...
import { TableSymbol, EnumSymbol, ColumnSymbol, EnumFieldSymbol, SchemaSymbol } from '@/core/binder/symbol/symbols';
...
describe('[example] records binder', () => {
  ...
  test('should bind records with schema-qualified table', () => {
    ...
    const authSchemaRefs = compiler.bindProject().getValue().symbolToReferences.get(authSchema) ?? [];
  ...
  test('should detect unknown table in records', () => {
    ...
    const errors = compiler.bindProject().getErrors();
