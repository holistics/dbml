...
describe('[example] binder - check with enum', () => {
  test('should bind simple enums in inline checks of tables', () => {
    const source = `
Enum status { active }
...
`;
  ...
  test('should bind simple enums in inline checks of table partials', () => {
    const source = `
Enum status { active }
TablePartial users { id varchar [check: status] }
`;
    const result = analyze(source);
  ...
  test('should allow schema-qualified enums in inline checks of table partials', () => {
    const source = `
Enum schema.status { active }
TablePartial users { id varchar [check: schema.status] }
`;
    const result = analyze(source);
