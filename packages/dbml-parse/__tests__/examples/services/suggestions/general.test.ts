...
describe('[example] CompletionItemProvider', () => {
  ...
  describe('should suggest in Ref block', () => {
    ...
    it('- should suggest columns after dot on rhs of relationship operator when table is imported via use', () => {
      ...
      layout.setSource(baseFp, baseContent);
      layout.setSource(consumerFp, consumerContent);
      const compiler = new Compiler(layout);
...
describe('[example] CompletionItemProvider - Records Row Snippets', () => {
  ...
  describe('should work with partial table injection', () => {
    ...
    it('- should work with explicit column list in records with partial table injection', () => {
      ...
      const recordSnippet = result?.suggestions?.find((s) => s.label === 'Record row snippet');
      expect(recordSnippet).toBeDefined();
      const insertText = recordSnippet?.insertText as string;
      expect(insertText).toContain('id (int)');
      expect(insertText).toContain('name (varchar)');
      expect(insertText).toContain('price (decimal)');
      expect(insertText).not.toContain('created_at');
    });

    it('- should work with top-level Records referencing table with partial injection', () => {
      const program = `
        TablePartial common {
          id int
          updated_at timestamp
        ...
        Records orders(id, total, updated_at) {
      ...
      const recordSnippet = result?.suggestions?.find((s) => s.label === 'Record row snippet');
      expect(recordSnippet).toBeDefined();
      expect(recordSnippet?.insertText).toEqual('${1:id (int)}, ${2:total (decimal)}, ${3:updated_at (timestamp)}');
    });

    it('- should handle multiple partial injections in correct order', () => {
      const program = `
      ...
      expect(recordSnippet).toBeDefined();
      const insertText = recordSnippet?.insertText as string;
      // Should include all columns: injected from id, regular column, and injected from timestamps
      expect(insertText).toContain('id (int)');
      expect(insertText).toContain('title (varchar)');
      expect(insertText).toContain('created_at (timestamp)');
      expect(insertText).toContain('updated_at (timestamp)');
    });
  });
